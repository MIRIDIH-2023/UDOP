...
class CurriculumTrainer(Trainer):
  ...
  def compute_loss(self, model, inputs, return_outputs=False):
      if self.loss_fct is None:
        ...
        loc_location = torch.where(mask_mse>=-1,True,False).to('cuda') #mask부분
        loc_indices = []
        # sliding window 형식으로 검사하기
        for batch in range(mask_mse.size(0)):
            for i in range( 1, len(loc_location[batch]) - 4 + 1, 5 ):
                window = loc_location[batch, i:i + 4]
                loc_indices.extend([False])

                if torch.all(window == True): # if four <loc_x> tokens are correct
                    loc_indices.extend([True,True,True,True])
                else:
                    loc_indices.extend([False,False,False,False])
        loc_indices = torch.Tensor(loc_indices).bool().to('cuda')
        
        ce_loss_fct = nn.CrossEntropyLoss(ignore_index=-100)
        ce_loss = ce_loss_fct(logits.view(-1, logits.size(-1)) [~loc_indices]  , labels.view(-1) [~loc_indices]  )
        ##################### 끝 ########################
        

        # If there are any location tokens in the batch, compute MSE loss
        if torch.any(mask_mse):
          input = max_logits_indices.view(-1)[loc_indices].float().requires_grad_()
          target = labels.view(-1)[loc_indices].float()
        ...
        ##############################################
          elif self.loss_fct =='IOU':
            IOU_loss_class = losses.IOU() ###### 이거 바꿔주세여 ############
            target = labels.view(-1)[loc_indices].int() 
            loc_loss = IOU_loss_class(input,target)
        ##############################################
  ...
  def evaluation_loop(
      ...
      custom_metrics = metric.compute_custom_metrics(model, eval_dataset)
